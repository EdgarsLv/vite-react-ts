...
import { Children, Items } from './NavConfig'
...
function NavItemRoot({ item }: { item: Items }) {
  ...
  const renderContent = (
    <>
      <ListItemIcon sx={{ color: active ? (theme) => theme.palette.info.main : 'inherit' }}>
        <Icon />
      </ListItemIcon>
  ...
  return (
    <ListItemButton
      sx={{
        '&.active': { color: (theme) => theme.palette.info.main },
      }}
      onClick={handleMenu}
...
function ListSubItem({ title, path }: Children) {
  ...
  return (
    <ListItemButton
      sx={{ pl: 2, '&.active': { color: (theme) => theme.palette.info.main } }}
      onClick={handleMenu}
      component={RouterLink}
      to={path}
      end
...
function DotIcon({ active }: { active: boolean }) {
  const activeStyle: SxProps<Theme> = { color: 'info.main', width: '10px' }
